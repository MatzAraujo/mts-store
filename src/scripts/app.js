...
function initMobileNav() {
  ...
  hamburger.addEventListener("click", () => {
    ...
    // ensure panel sits under header when opening
...
function initHeroAnimations() {
  ...
  tl.to(heroMedia, { autoAlpha: 1, scale: 1.03, duration: 1.2 }, 0)
    .fromTo(
      chars,
      { y: 24, autoAlpha: 0 },
      { y: 0, autoAlpha: 1, stagger: 0.02, duration: 0.8 },
      0.12,
    )
...
function initProductHoverSwap() {
  ...
  cards.forEach((card) => {
    ...
    card.addEventListener("mouseenter", () => {
      ...
      gsap.to(img1, { autoAlpha: 0, duration: 0.48, ease: "power3.out" });
      gsap.to(img2, { autoAlpha: 1, duration: 0.48, ease: "power3.out" });
    });

    card.addEventListener("mouseleave", () => {
      gsap.killTweensOf([img1, img2]);
      gsap.to(img1, { autoAlpha: 1, duration: 0.48, ease: "power3.out" });
      gsap.to(img2, { autoAlpha: 0, duration: 0.48, ease: "power3.out" });
    });

    card.parentElement.addEventListener("focusin", () => {
      gsap.to(img1, { autoAlpha: 0, duration: 0.28, ease: "power3.out" });
      gsap.to(img2, { autoAlpha: 1, duration: 0.28, ease: "power3.out" });
    });
    card.parentElement.addEventListener("focusout", () => {
      gsap.to(img1, { autoAlpha: 1, duration: 0.28, ease: "power3.out" });
      gsap.to(img2, { autoAlpha: 0, duration: 0.28, ease: "power3.out" });
    });
  });
}
...
function initCartDrawer() {
  ...
  function openCart() {
    ...
    const focusable = cartDrawer.querySelector(
